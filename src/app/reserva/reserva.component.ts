...
export class ReservaComponent implements OnInit {
  ...
  // VARIABLE PARA CONTROLAR EL MÁXIMO DE PASAJEROS SEGÚN EL AUTO
  opcionesPasajeros: number[] = [1, 2, 3, 4];

  // NUEVA VARIABLE PARA CONTROLAR EL MODAL
  ...
  // FUNCIÓN PARA CERRAR EL MODAL DE ÉXITO Y LIMPIAR LA URL
  closeSuccessModal() {
  ...
  constructor(private fb: FormBuilder, private cdr: ChangeDetectorRef) {
    this.reservationForm = this.fb.group({
      ...
      vehiculo: ['', Validators.required], destino: ['', Validators.required], // <-- Cambiado a vacío y requerido
  ...
  ngOnInit() {
    // NUEVA LÓGICA: ESCUCHAR CAMBIOS EN EL VEHÍCULO PARA AJUSTAR PASAJEROS
  ...
  async validarHorarioDisp(fecha: string, hora: string, campoHora: string) {
    ...
    try {
      ...
      if (data) {
        ...
        if (overlaps >= 3) {
          // ABRIMOS EL MODAL BONITO EN LUGAR DEL ALERT NATIVO
    ...
    if (this.reservationForm.valid) {
      ...
      if (this.cotizacion === null) {
        ...
        supabase.functions.invoke('openpay-checkout', { body: { tipoAccion: 'WHATSAPP_COTIZACION', nombre: nombreCompleto, email: form.email, monto: this.cotizacion, descripcion: form.destino, telefono: telLimpio, idioma: this.lang } }).catch();
          ...
          this.cdr.detectChanges(); 
          try {
            const urlRetorno = window.location.origin + '/reserva';
            const datosPago = {
              monto: this.cotizacion, nombre: `${this.reservationForm.value.nombres} ${this.reservationForm.value.apellidos}`,
              email: this.reservationForm.value.email, descripcion: `Traslado Ejecutivo Vancity`, redirectUrl: urlRetorno
            };
